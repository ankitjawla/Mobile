...
public class PeerInfoProvider extends ContentProvider {
	...
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			Log.w(ChatApp.TAG, "Upgrading database from version " + oldVersion
	...
	public Uri insert(Uri uri, ContentValues initialValues) {
		...
		if (rowId > 0) {
			Uri peerUri = ContentUris.withAppendedId(
					ChatContent.Peers.CONTENT_URI, rowId);
	...
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		...
		case PEER_ID:
			...
			count = db.delete(PEER_TABLE_NAME, BaseColumns._ID
					+ "="
					+ id
	...
	public int update(Uri uri, ContentValues values, String selection,
		...
		case PEERS:
			count = db
					.update(PEER_TABLE_NAME, values, selection, selectionArgs);
			break;

		case PEER_ID:
			String id = uri.getPathSegments().get(1);
			count = db.update(PEER_TABLE_NAME, values, BaseColumns._ID
					+ "="
					+ id
					+ (!TextUtils.isEmpty(selection) ? " AND (" + selection
							+ ")" : ""), selectionArgs);
	...
	static {
		...
		peerProjectionMap.put(BaseColumns._ID, BaseColumns._ID);
		...
		peerProjectionMap.put(ChatContent.Peers.LATITUDE,
				ChatContent.Peers.LATITUDE);
		peerProjectionMap.put(ChatContent.Peers.LONGITUDE,
