...
public class MessageProvider extends ContentProvider {
	...
	/*
	 * Note: We should use a cursor that implements a circular buffer and drops
	 * all but the last N messages received. As it is, this buffer grows
	 * unboundedly, a potential DoS attack.
	 */
	...
	private MatrixCursor loadMessages() {
		...
		if (messages == null) {
			...
			String[] columns = { BaseColumns._ID, ChatContent.Messages.SENDER,
			...
			try {
				...
				BufferedReader messageInputFile = new BufferedReader(
				...
				Log.i(ChatApp.TAG, "Messages file has not been created yet.");
			} catch (IOException e) {
				Log.e(ChatApp.TAG, "IO error while reading message file");
			}
		}
	...
	private void saveMessages(Cursor messages) {
		...
		try {
			OutputStream os = getContext().openFileOutput(DATABASE_NAME,
					Context.MODE_PRIVATE);
			BufferedWriter messageOutputFile = new BufferedWriter(
					new OutputStreamWriter(os));
			for (int row = 1; row <= messages.getCount(); row++) {
				String sender = messages.getString(messages
						.getColumnIndex(ChatContent.Messages.SENDER));
				String message = messages.getString(messages
						.getColumnIndex(ChatContent.Messages.SENDER));
			...
			Log.e(ChatApp.TAG, "IO error while writing message file");
		}
	...
	public Uri insert(Uri uri, ContentValues initialValues) {
		...
		String[] message =  {String.valueOf(id),values.get(ChatContent.Messages.SENDER).toString(), values.get(ChatContent.Messages.MESSAGE).toString()};
